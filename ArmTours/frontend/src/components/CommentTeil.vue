<template>
  <div class="comments">
    ...
    <div class="comment-form">
      <textarea v-model="localComment" placeholder="Schreibe einen Kommentar"></textarea>
      <div class="tooltip">
        <span class="tooltiptext" v-if="showCommentWarning">Um einen Kommentar abzugeben, müssen Sie sich
          registrieren.</span>
        <button @click="addComment" class="submit-comment-button">Kommentar senden</button>
      </div>
    </div>
    <div v-if="localComments.length > 0">
      <div v-for="comment in localDisplayedComments" :key="comment.id" class="comment">
        <div class="comment-header">
          ...
          <div class="comment-date">{{ formatDate(comment.created_at) }}
            <div class="rating">
              <div class="tooltip">
        ...
        <button v-if="canDeleteComment" @click="deleteComment(comment.id)" class="delete-comment-button">Kommentar
          löschen</button>
		...
		l-1.99-18c-0.132-1.675,0.914-2,1.99-2h17c2.209,0,4-1.791,4-4C62,33.79,60.209,32,58,32z" />
            ...
            <button @click="deleteReply(reply.id)" class="delete-comment-button">Antwort löschen</button>
            ...
            <div v-if="localReplyOnReplyFormVisible[reply.id]" class="reply-form">
              <textarea v-model="localReplyOnReplies[reply.id]" placeholder="Antwort auf Antwort schreiben..."></textarea>
              <div class="tooltip">
                <span class="tooltiptext" v-if="showCommentWarning">Um einen Kommentar abzugeben, müssen Sie sich
                  registrieren.</span>
      ...
      <button v-if="localComments.length > 3" @click="$emit('openAllCommentsPopup')" class="show-all-comments-button">
...
<style scoped> .comments {
 ...
 .comment-form:hover .tooltiptext {
   visibility: visible;
   opacity: 1;
 }
</style>
